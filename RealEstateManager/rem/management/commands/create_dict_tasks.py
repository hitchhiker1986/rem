...
class Command(BaseCommand):
    ...
    def handle(self,  *args, **options):
        if options['utility_id'] > 0:
            try:
                utility = Utility.objects.get(id=options['utility_id'])
            except Utility.DoesNotExist:
                raise CommandError('Utility does not exist')
            ...
            if datetime.date.today() - next_check_start_date <= 5:
                task = ToDo()
                task.start_day = next_check_start_date
                task.end_day = next_check_end_date
                task.description = str(Utility.serial)
                    + " sorozatszámú fogyasztásmérö óraállásának diktálása. Esedékes "
                    + str(next_check_end_date) + " dátumig!"
